...
def preencher_word(dados_organizados):
    ...
    try:
        ...
        # Varre todas as tabelas e todas as linhas procurando os cabeçalhos
        encontrou = False
        ...
        if not tabela_alvo:
            print("\nERRO CRÍTICO: Não encontrei nenhuma linha com 'Históricos' e 'Profetas'.")
            print("Verifique se o documento Word tem exatamente esses nomes escritos.")
            return

        # Preenchendo a tabela
        max_linhas = max(len(dados_organizados[cat]) for cat in colunas_idx.keys())
        
        # O número de linhas disponíveis para dados começa APÓS o cabeçalho
        linhas_totais = len(tabela_alvo.rows)
        linhas_dados_existentes = linhas_totais - (indice_cabecalho + 1)
        
        # Adiciona linhas novas se necessário
        ...
        for cat_nome, lista_leituras in dados_organizados.items():
            ...
            if idx_coluna == -1: continue # Segurança caso não ache uma coluna específica
            
            for i, leitura in enumerate(lista_leituras):
                # A linha onde escrevemos é: Linha do Cabeçalho + 1 + i
                linha_destino = indice_cabecalho + 1 + i
                cell = tabela_alvo.rows[linha_destino].cells[idx_coluna]
                
                # Limpa se tiver algo escrito (opcional) e escreve o novo
                cell.text = leitura
...
def main():
    print("\n" + "="*50)
    print(" ORGANIZADOR LITÚRGICO & WORD (V2)")
    print("="*50)
    
    leituras_raw = []
    
    print("Digite as leituras (ex: 'Is 50,4'). Enter vazio para terminar.")
    while True:
        entrada = input(">> ")
        if entrada.strip() == "" or entrada.lower() == "sair":
            break
        leituras_raw.append(entrada)

    if not leituras_raw:
        print("Nenhuma leitura inserida.")
        return
    ...
    resp = input("\nDeseja gerar o documento Word? (s/n): ")
    if resp.lower() == 's':
        preencher_word(organizacao)
    else:
        print("Finalizado.")
